...
Future<void> runBot(AppDatabase db) async {
  ...
  bot.onCallbackQuery().listen((query) async {
    try {
      ...
      await bot.telegram.answerCallbackQuery(
        query.id,
...
Future<void> _handleCallback({
  ...
  if (data.startsWith('category_')) {
    ...
    if (categoryId == null) {
      await bot.telegram.answerCallbackQuery(query.id, text: 'Danh mục không hợp lệ');
      return;
    }

    final category = db.getCategoryWithStock(categoryId);
    if (category == null) {
      await bot.telegram.answerCallbackQuery(query.id, text: 'Danh mục không tồn tại');
      return;
    }
  ...
  if (data.startsWith('buy_')) {
    ...
    if (categoryId == null) {
      await bot.telegram.answerCallbackQuery(query.id, text: 'Dữ liệu không hợp lệ');
      return;
    }
  ...
  if (data.startsWith('confirm_buy_')) {
    ...
    if (!result.ok) {
      ...
      await bot.telegram.answerCallbackQuery(query.id, text: result.message, showAlert: true);
      return;
    }
    ...
    await bot.telegram.answerCallbackQuery(query.id, text: 'Mua thành công');
    return;
  }

  await bot.telegram.answerCallbackQuery(query.id, text: 'Hành động không hỗ trợ');
}
